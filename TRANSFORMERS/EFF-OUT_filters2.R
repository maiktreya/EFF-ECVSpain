...
##### NAMES CONTAINERS
source("SOURCE/GENERATORS/SELECTORS/names-filters2.R")

## CONVERSION TO FACTORIAL, TREATMENT, AND DEBT RATIOS DEFINITIONS
source("SOURCE/GENERATORS/SELECTORS/EFF_treatment_filters.R")
...
filters_depo <- rbind(filters_depo3, fill = TRUE)[, year := sel_year][, reduced := reduced]
...
survey_weights$variables[, "p2_1" == 1]
## MULTI DIMENSIONAL CONTINGENCY TABLES COMBINING CLASS AND AGE
filter_multi1 <- svyby(~p2_1, ~nsitlabdom, survey_weights, svymean, keep.var = F, na.rm = reduced) %>% data.table()
filter_multi1 <- filter_multi1[, year := sel_year][, var := "p2_1_cesion"][, reduced := as.character(reduced)]
filter_multi2 <- svyby(~p2_2, ~  bage, survey_weights, svymean, keep.var = F, na.rm = reduced) %>% data.table()
filter_multi2 <- filter_multi2[, year := sel_year][, var := "p2_2_inheritance"][, reduced := as.character(reduced)]
filter_multi3 <- svyby(~main_other, ~ nsitlabdom + bage, survey_weights, svymean, keep.var = F, na.rm = reduced) %>% data.table()
...
filter_multi <- rbind(filter_multi1, filter_multi2, filter_multi3, filter_multi4, filter_multi5, fill = T)

svyq <- function(...) {
    tryCatch(survey::svyquantile(...), error = function(e) matrix(NA, 1, 1))
} # nolint
survey_weights_var <- subset(survey_weights, bage != 6 & nsitlabdom == 1)
var_multi1 <- svyby(~valor, ~ nsitlabdom + bage, survey_weights_var, svyq, quantiles = .5, na.rm = T)[1:3] %>% data.table()
var_multi2 <- svyby(~riquezafin, ~ nsitlabdom + bage, survey_weights_var, svyq, quantiles = .5, na.rm = T)[1:3] %>% data.table()
var_multi <- rbind(var_multi1, var_multi2, use.names = F)[, year := sel_year][, type := reduced]


#### gini analysys
survey_weights_gini <- subset(survey_weights, nsitlabdom == 1)
gini_val <- convey::svygini(~valor, convey::convey_prep(survey_weights_gini), na.rm = T)
gini_fin <- convey::svygini(~riquezafin, convey::convey_prep(survey_weights_gini), na.rm = T)
gini_val_list <- rbind(gini_val_list, gini_val)
gini_fin_list <- rbind(gini_fin_list, gini_fin)
...
wealth_filters8 <- rbind(wealth_filters8, var_multi)